...
public class AccDao {
	...
	public AccDto getData(int accno) {
		...
		try {
			...
			// 실행할 sql 문
			String sql = "select * from acc where accno = ?";
	...
	public List<AccDto> getList() {
		...
		try {
			...
			conn = new DBConnect().getconn();
			// 실행할 sql 문
			String sql = "select accno,TO_CHAR(da ,'YYYY-MM-DD')da,inc,outc,con from acc order by da";
			// sql 문을 대신 실행해줄 PreparedStatement 객체의 참조값 얻어오기
			pstmt = conn.prepareStatement(sql);
			// sql 문이 ? 가 존재하는 미완성이라면 여기서 완성한다.
	...
	public static boolean update(AccDto dto) {
		...
		try {
			...
			conn = new DBConnect().getconn();
			// 실행할 sql 문
			String sql = "update acc set da = ? , inc = ?, outc = ?, con = ? where accno = ?";
			// sql 문을 대신 실행해줄 PreparedStatement 객체의 참조값 얻어오기
			pstmt = conn.prepareStatement(sql);
			// sql 문이 ? 가 존재하는 미완성이라면 여기서 완성한다.
	...
	public boolean insert(AccDto dto) {
		...
		try {
			...
			e.printStackTrace();
			JOptionPane.showMessageDialog(null, "입력한 데이터가 올바르지 않습니다");
		} finally { // 예외가 발생을 하던 안하던 실행이 보장되는 블럭에서 마무리 작업
			try {
				if (pstmt != null)
					pstmt.close();
